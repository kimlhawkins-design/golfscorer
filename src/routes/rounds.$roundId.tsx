...
function RoundPage() {
  ...
  const holeSectionRef = useRef<HTMLDivElement | null>(null);
  ...
  const scrollToSection = (section: "hole" | "scores" | "gps" | "card") => {
    if (section === "scores") {
      jumpToScoreEntry();
      return;
    }
    ...
    const target = section === "hole" ? holeSectionRef.current : scorecardRef.current;
  ...
  return (
    <div className="min-h-screen app-bg-green">
      <div className="max-w-3xl mx-auto px-4 pb-28 pt-8 sm:pb-8">
        ...
        {/* Current Hole */}
        <div ref={holeSectionRef} className="scroll-mt-4">
        <CurrentHoleControl
          hole={selectedHole}
          par={selectedPar}
                ...
                return (
                  <div key={p.id} className="rounded-xl border border-white/10 p-3 shadow-sm shadow-black/20" style={{ backgroundColor: "#242a28" }}>
                    ...
                    <div className="mt-3 grid grid-cols-[60px_1fr_60px] items-center gap-3">
                      <button
                        type="button"
                        aria-label={"Decrease " + p.name + " putts"}
                        onClick={() => {
                      ...
                      <div className="h-12 rounded-xl border border-white/10 text-center" style={{ backgroundColor: "#101413" }}>
                        <div className="pt-1 text-sky-200/60 text-[10px] uppercase tracking-wider">Putts</div>
        ...
        {/* Scorecard */}
        <div ref={scorecardRef} className="scroll-mt-4 app-panel backdrop-blur border rounded-2xl overflow-hidden mb-6">
        ...
        <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-[#101413]/95 px-2 pb-[env(safe-area-inset-bottom)] pt-2 shadow-2xl shadow-black/40 backdrop-blur sm:hidden" aria-label="Round navigation">
          <div className="mx-auto grid max-w-3xl grid-cols-5 gap-1">
            <button type="button" onClick={() => scrollToSection("hole")} className="flex min-h-[58px] flex-col items-center justify-center gap-1 rounded-xl text-white/70 hover:bg-white/10 hover:text-white">
              <Flag className="h-5 w-5" aria-hidden="true" />
              <span className="text-[11px] font-bold">Hole</span>
            </button>
            <button type="button" onClick={() => scrollToSection("scores")} className="flex min-h-[58px] flex-col items-center justify-center gap-1 rounded-xl bg-lime-400 text-slate-950 shadow-lg shadow-lime-400/10">
              <PencilLine className="h-5 w-5" aria-hidden="true" />
              <span className="text-[11px] font-black">Scores</span>
            </button>
            <button type="button" onClick={() => scrollToSection("gps")} className="flex min-h-[58px] flex-col items-center justify-center gap-1 rounded-xl text-white/70 hover:bg-white/10 hover:text-white">
              <MapPin className="h-5 w-5" aria-hidden="true" />
              <span className="text-[11px] font-bold">GPS</span>
            </button>
            <button type="button" onClick={() => scrollToSection("card")} className="flex min-h-[58px] flex-col items-center justify-center gap-1 rounded-xl text-white/70 hover:bg-white/10 hover:text-white">
              <ClipboardList className="h-5 w-5" aria-hidden="true" />
              <span className="text-[11px] font-bold">Card</span>
            </button>
            <Link to="/rules" className="flex min-h-[58px] flex-col items-center justify-center gap-1 rounded-xl text-white/70 hover:bg-white/10 hover:text-white">
              <ScrollText className="h-5 w-5" aria-hidden="true" />
              <span className="text-[11px] font-bold">Rules</span>
            </Link>
          </div>
        </nav>
    </div>
